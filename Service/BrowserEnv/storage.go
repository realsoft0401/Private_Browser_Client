...
func writeEnvPackageFiles(envPath string, paths model.ManifestPaths, files envPackageFiles) error {
	if err := writeJSONFile(filepath.Join(envPath, "manifest.json"), files.Manifest); err != nil {
		return err
	}
	writeTasks := []struct {
		relativePath string
