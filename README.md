...
| DELETE | `/api/v1/edge/browser-envs/:envId/del` | 删除环境包关联的本机 Docker 镜像，不删除环境包资产 |
| DELETE | `/api/v1/edge/browser-envs/:envId/package` | SSE 任务：彻底删除环境包，删除配置目录、登录态目录、已停止容器和 SQLite 索引，不删除 Docker 镜像 |
...
| `DELETE /api/v1/edge/browser-envs/:envId/del` | 删除关联运行镜像 | 只读取 `profile.runtime.image` 并删除本机 Docker image，不碰环境包目录、登录态目录和 SQLite |
| `DELETE /api/v1/edge/browser-envs/:envId/package` | 彻底销毁环境资产 | 删除环境包目录、登录态目录、已停止容器和 SQLite 索引，不删除 Docker 镜像 |
...
- `DELETE /api/v1/edge/browser-envs/:envId/package`：彻底销毁环境资产，删除后不可恢复。
...
```bash
curl -X DELETE http://127.0.0.1:3300/api/v1/edge/browser-envs/{envId}/del
```

该接口从 `profile.runtime.image` 读取镜像名，只删除本机 Docker image；不会删除环境包目录、`browser-data/profile`、proxy/fingerprint、container.json 或 SQLite 索引。
如果环境包仍在运行中，接口会返回状态冲突，调用方应先执行 `stop`。
如果镜像仍被其它容器占用，Docker 会拒绝删除，接口应返回可排查的错误或 warning。

彻底删除浏览器环境包示例：

```bash
curl -X DELETE http://127.0.0.1:3300/api/v1/edge/browser-envs/{envId}/package
```

package 删除接口会物理删除环境包目录，包括 `profile.json`、`binding.json`、`proxy/`、`fingerprint/` 和 `browser-data/profile`，同时删除关联的已停止 Docker 容器并移除 SQLite `browser_envs` 索引记录。
...
```bash
IMAGE=crpi-6s60spbjvluac8j8.cn-shanghai.personal.cr.aliyuncs.com/ln0216/private_browser_edge_server:0.1.9-amd64 \
...
脚本会生成并挂载 `/Business/data/config-docker-host.yaml` 到容器内 `/app/Settings/config-docker.yaml`，其中 Docker API 地址是：
...
```bash
IMAGE=crpi-6s60spbjvluac8j8.cn-shanghai.personal.cr.aliyuncs.com/ln0216/private_browser_edge_server:0.1.9-amd64
CONTAINER_NAME=private-browser-edge-server
DATA_DIR=/Business/data
...
docker inspect private-browser-edge-server \
...
```bash
docker run -d \
  --name private-browser-edge-server \
  --label bv.project=private-browser-client \
  --label bv.role=edge-service \
  --restart unless-stopped \
  --network host \
  -v "/Business/data:/app/data" \
  -v "/Business/data/config-docker-host.yaml:/app/Settings/config-docker.yaml:ro" \
  --cap-add NET_ADMIN \
  --device /dev/net/tun:/dev/net/tun \
  crpi-6s60spbjvluac8j8.cn-shanghai.personal.cr.aliyuncs.com/ln0216/private_browser_edge_server:0.1.9-amd64
```
...
```bash
docker stop private-browser-edge-server
docker rm private-browser-edge-server
```
